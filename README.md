...
2. Подождать пару секунд (сообщение уйдёт в Kafka, Data Service его обработает).

3. **Посмотреть записи**:

   ```bash
   curl "http://localhost:8080/api/appointments?patientName=Иванов"
   ```

4. **Посмотреть отчёты**:

   ```bash
   curl http://localhost:8080/api/reports/appointments-per-day
   curl http://localhost:8080/api/reports/top-patients
